...
`SHP`, `CreateContour`, `Building` 등 일부 기능은 `shapefile.py` 또는 `pyshp` 모듈이 필요합니다.

SHP 기능이 실행되지 않으면 Rhino Python 환경에서 `shapefile` 모듈을 불러올 수 있는지 확인하세요.
